...
function getBirderData(birder, startingId) {
  ...
  let cldata = getCuratedList(birder)
  ...
  for (let i = startingId; i < startingId + birdsPerPage && i < lldata.length; i++) {
    ...
    birdData.curated = birdName in cldata ? cldata[birdName] : null
...
// Returns the file path for a user's curated list
function curatedListPath(birder) {
  const clname = 'curated_list.json'
  return path.join(dataDirectory, birder, clname)
}

// Loads and returns the curated photo preferences of a user
export function getCuratedList(birder) {
  let cldata = {}
  const clpath = curatedListPath(birder)
  if (fs.existsSync(clpath)) {
    try {
      let clstring = fs.readFileSync(clpath, 'utf8')
      cldata = JSON.parse(clstring)
    } catch (err) {
      console.log(err)
    }
  }
...
// Updates and saves a user's curated photo preferences
export function setCuratedPreference(birder, birdName, photoId) {
  let cldata = getCuratedList(birder)
  cldata[birdName] = photoId
  const clpath = curatedListPath(birder)
  try {
...
// Sets a user's curated preference for a bird to null
export function clearCuratedPreference(birder, birdName) {
  setCuratedPreference(birder, birdName, null)
}
