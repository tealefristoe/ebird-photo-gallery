...
import {bk, checklistUrl, specimenUrl} from './constants'

// Functions for displaying photo information

// Displays a photo's community rating information
...
// These functions display a photo's location and date in various formats
...
// Returns links to ebird and Macaulay library
export function photoLinksDisplay(photoData) {
  if (photoData) {
    return (
      <div>
        <div>{photoData[bk.photoDate] || photoData[bk.photoLocality] || photoData[bk.photoChecklistId] ?
          <a href={checklistUrl(photoData[bk.photoChecklistId])} target="_blank">eBird Checklist</a> :
        ...
        <div><a href={specimenUrl(photoData[bk.photoId])} target="_blank">Macaulay Library</a></div>
      </div>
    )
  }
  return <span>No photo data</span>
...
// Returns a link to a species page, listing the number of photos a species has
export function curatedLink(user, photoData) {
  // <a target="_blank">{photoData.speciesData.photos.length + ' photo' + (photoData.speciesData.photos.length != 1 ? 's' : '')}</a>
  return (
    <Link href={'/species?user=' + escapeSpaces(user) + '&bird=' + escapeSpaces(photoData.speciesData[bk.lifeBirdName])}>
      <a target="_blank">{'All photos (' + photoData.speciesData.photos.length + ')'}</a>
    </Link>
  )
}
